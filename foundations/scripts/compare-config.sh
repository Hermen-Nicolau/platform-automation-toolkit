...
rm -rf ../${iaas}/${INITIAL_FOUNDATION}/config/defaults/${product}-new.yml
touch ../${iaas}/${INITIAL_FOUNDATION}/config/defaults/${product}-new.yml

# ---- Add default vars to defaults file
if [ -f ${wrkdir}/default-vars.yml ]; then
  vars=$(cat ${wrkdir}/default-vars.yml | tr -d '[:space:]')
  if [[ "${vars}" != "" && "${vars}" != "{}" ]]; then
    cat ${wrkdir}/default-vars.yml >> ../${iaas}/${INITIAL_FOUNDATION}/config/defaults/${product}-new.yml
  fi
fi
...
# ---- We need to make changes to the file structure to accomplish this. We are going to split between 
